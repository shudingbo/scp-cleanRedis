...
Handler.prototype.run = function(sc, job){
...
//! Stop
Handler.prototype.stop = function(sc, job){
	this._isRun = false;
	if( this.redis !== null ){
...
Handler.prototype.cleanHashs = function( keyInfo,cb ){
	var self = this;
    ...
    function( keys, callback) {
		self.cleanHashSignal( keys, param,callback );
    },
	], function (err, result) {
		cb( err );
	});
...
Handler.prototype.cleanHashSignal = function( keys, param, cb ){
	var self = this;
	...
	async.during(
		function (callback) {
			return callback(null, idx < len);
		},
		function (callback) {
			//console.log('--- ', idx, len, keys[idx]);
			self.redis.hkeys( keys[idx],function( err, rep ){
				if( !err ){
					var lenRep = rep.length;

					var i = 0;
					for( i=0; i<lenRep; i++ ){
						if( checkParams( rep[i], param ) === true ){
							self.redis.hdel( keys[idx], rep[i] );
							//console.log( ' Del ', rep[i] );
						}
					}
				}
